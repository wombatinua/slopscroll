...
When feed mode is `offline_video` or `offline_image`, this spec is not used for feed serving.
...
- Offline mode enabled (`offline_video` or `offline_image`): spec reload remains intentionally blocked with `409`.
